#!/bin/sh
# Verifies pimd-dense operation in a single router setup:
#  - forwardng multicast between two emulated end devices
#  - PIM Hello on both LANs
...
../src/pimdd -f "/tmp/$NM/conf" -n -p "/tmp/$NM/pid" -l debug -u "/tmp/$NM/sock" &
sleep 1
...
lines1=$(tshark -r "/tmp/$NM/left.pcap" 2>/dev/null | grep "PIMv2 56 Hello" | tee "/tmp/$NM/result" | wc -l)
lines2=$(tshark -r "/tmp/$NM/left.pcap" 2>/dev/null | grep "IGMPv3 50 Membership Query" | tee -a "/tmp/$NM/result" | wc -l)
cat "/tmp/$NM/result"
echo " => $lines1 PIM Hello, expected 1"
...
lines1=$(tshark -r "/tmp/$NM/right.pcap" 2>/dev/null | grep "PIMv2 56 Hello" | tee "/tmp/$NM/result" | wc -l)
lines2=$(tshark -r "/tmp/$NM/right.pcap" 2>/dev/null | grep "IGMPv3 50 Membership Query" | tee -a "/tmp/$NM/result" | wc -l)
cat "/tmp/$NM/result"
echo " => $lines1 PIM Hello, expected 1"
echo " => $lines2 IGMP Query, expected 1"
# shellcheck disable=SC2086 disable=SC2166
